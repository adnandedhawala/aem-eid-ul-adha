...
export const GoshList: FC<{ tokenList: any[], handleChangeStatus: any }> = ({ tokenList, handleChangeStatus }) => {
    ...
    useEffect(() => {
        let statusFilteredList = tokenList.filter(val => val[TOKEN_COUNTER.GOSH] === activeState);
        if(activeState === STATUS.PENDING){
            statusFilteredList= tokenList.filter(val => val[TOKEN_COUNTER.ZABIHAT] === STATUS.COMPLETED).filter(val => val[TOKEN_COUNTER.GOSH] === activeState)
        }
        if (!searchText) {
                                ...
                                <p className="text-4xl text-yellow-500">
                                    {
                                        tokenList.filter(
                                            ...
                                            (val) => val[TOKEN_COUNTER.GOSH] === STATUS.PENDING
                                        ).length
                                    }
                                </p>
                            </div>
                        ...
                        <div className="flex">
                            ...
                            <div className="flex flex-col flex-grow">
                                <p className="text-4xl text-green-500">
                                    {
                                        tokenList.filter(
                                            (val) => val[TOKEN_COUNTER.GOSH] === STATUS.COMPLETED
                                        ).length
                                    }
                                </p>
                            </div>
                                ...
                                <Row gutter={[4, 4]}>
                                    ...
                                    <Col xs={24} className="text-center">
                                        <Button
                                            ...
                                            onClick={() => handleChangeStatus(value.id, TOKEN_COUNTER.GOSH, activeState === STATUS.PENDING ? STATUS.COMPLETED : STATUS.PENDING)}
                                        >
                                            {
