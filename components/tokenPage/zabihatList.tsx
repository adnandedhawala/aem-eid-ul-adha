...
export const ZabihatList: FC<{ tokenList: any[], handleChangeStatus: any }> = ({ tokenList, handleChangeStatus }) => {
    ...
    useEffect(() => {
        const statusFilteredList = tokenList.filter(val => val[TOKEN_COUNTER.ZABIHAT] === activeState);
        if (!searchText) {
    ...
    return (
        <>
            <h1 className="font-semibold text-xl mt-8 mb-2"><span>Total : </span><span>{tokenList.length}</span></h1>
            <Row gutter={[16, 16]}>
                        ...
                        <div className="flex">
                            ...
                            <div className="flex flex-col flex-grow">
                                <p className="text-4xl text-yellow-500">
                                    {
                                        tokenList.filter(
                                            (val) => val[TOKEN_COUNTER.ZABIHAT] === STATUS.PENDING
                                        ).length
                                    }
                                </p>
                            </div>
                                ...
                                <Row gutter={[4, 4]}>
                                    ...
                                    <Col xs={24} className="text-center">
                                        <Button
                                            ...
                                            onClick={() => handleChangeStatus(value.id, TOKEN_COUNTER.ZABIHAT, activeState === STATUS.PENDING ? STATUS.COMPLETED : STATUS.PENDING)}
                                        >
                                            {
